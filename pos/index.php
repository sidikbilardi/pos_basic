    ...
    <body class="skin-blue">
        ...
        <header class="header">
            ...
            <?php if(isset($_GET['meja']) != ''){ 
				...
				echo "<a href='index.php' class='logo'><strong>Willert</strong> POS</a>";
			}
			?>
		...
		$.ajax({
			...
			success: function(msg){
				...
				//alert("Pesanan berhasil ditambah");
                 ...
                 $("#button_trx").html('<a href="?page=review&meja='+id+'&trx='+msg+'"><div class="btn btn-success">Go to Transaction</div></a>');
...
/*	function ZReport(id){
		...
		$.ajax({
			type: "POST",
			url: "inc/proses_add.php",
			data: "action=printsZReport&id="+id,
			cache: false,
			success: function(msg){
                var msgg = 1;
                if(msgg == 1){
                    $("#z_detail").load("inc/zreport_detail_real.php");
                    alert("Closing day telah berhasil");
                }else if(msgg == 2){
                    alert("Masih ada transaksi yang open");
                }else{
                    alert(msgg);
                }
...
function UNBooktable(meja){
	...
	function doneBook(id,name,meja){
		//var meja = $( "#meja" ).val();
		var konfirmasi=confirm("Yakin Success booking "+name+" ? ");
		if (konfirmasi==true)
		{	
			$.ajax({
				type: "POST",
				url: "inc/proses_add.php",
				data: "action=bookingSuccess&id="+id+"&meja="+meja,
				cache: false,
				success: function(msg){
					window.location.reload(true);
					alert("Transaksi "+name+" berhasil ");
	...
	function cancelBook(id,name,meja){
		//var meja = $( "#meja" ).val();
		var act = '<?php echo $_GET['act']; ?>'
		var konfirmasi=confirm("Yakin Cancel booking "+name+" ? ");
		if (konfirmasi==true)
		{	
			$.ajax({
				type: "POST",
				url: "inc/proses_add.php",
				data: "action=bookingCancel&id="+id,
				cache: false,
				success: function(msg){
					if(act != ''){
						$("#tbl_book_list").load("table/table_booking_list.php?id="+meja); 
