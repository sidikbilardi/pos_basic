...
$query = mysql_fetch_array($q);
	if($query['cnt'] > 0 ){
		echo "<script>alert('Ada transaksi Open');</script>";
		$print = 'no';
	}else{
		...
		if($pt['pty'] > 0){
			echo "<script>alert('Ada Pettycash Open');</script>";
			$print = 'no';
		}else{
			...
			echo "<strong>Z - R E P O R T ".$now."</strong></br>";
			...
			while($zr = mysql_fetch_assoc($z)){
				...
				$total = 0; ?>
				<div class="col-xs-12">
					<?php echo $zr['kode_waiter'].' - '.$zr['nama_waiter'];?>
				
				</div> 
				<div class="col-xs-12">
					<div class="col-xs-6">Closing Total</div>
					<div class="col-xs-6">: <?php echo number_format($zr['close_nominal']);?></div>
				</div> 
...
<script>
	<?php if($print != 'no'){?>
	var printContent = document.getElementById('z_detail').innerHTML;
